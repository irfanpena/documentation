...
- **Background Customization**: Set the AI agent’s background to a green screen or use WebGL on the frontend to make it transparent or replace it with a custom background.
   - You can use WebGL on the frontend to make the green screen transparent or change its color. 
...
## Getting Started
You can create a conversation or an audio-only conversation by using the [Tavus Platform](https://platform.tavus.io/) or following the steps in the [Conversation API Quickstart](/sections/conversational-video-interface/persona/full-mode) guide.
