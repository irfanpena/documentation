...
A **Conversation** is a real-time video session between a user and a Tavus **Replica**. It enables two-way, face-to-face interaction through a fully managed WebRTC connection.

Tavus uses **Daily** to handle the underlying WebRTC infrastructure—including signaling, media transport, and room management—so you don’t need to manage any of it manually.
