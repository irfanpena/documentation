...
* **Model Layers**:
  ...
  * **TTS Configuration**:
    ...
    * Employs the `cartesia` voice engine with emotion control enabled to provide expressive, empathetic speech suitable for a doctor-like tone.

  * **STT Configuration**:

    * Uses Tavus’ advanced speech recognition engine with high sensitivity to both pauses and interruptions, plus smart turn detection for seamless real-time conversations.
</Tab>
...
* **Model Layers**
  ...
  * **Perception Configuration**
    ...
    * “Does the user appear distressed or uncomfortable?”
    ...
    If acne is detected, the persona is instructed to use the `acne_detected` tool, which reports visual findings using a boolean `have_acne` parameter. This adds real-time visual awareness to the dialogue.
