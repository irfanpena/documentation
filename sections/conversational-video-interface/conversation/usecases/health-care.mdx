...
* **Model Layers**
  ...
  * **LLM Configuration**: Uses the `tavus-llama` model with speculative inference. Includes the `get_skin_cures` function, which takes a `disease` input to provide specific treatment guidance.
...
<Note>
You can also explore our <a href="/sections/conversational-video-interface/sample-apps/health-consultation" target="_blank" rel="noopener noreferrer">Health Consultation</a> sample apps, which showcase how to use the Health Care personas in practice.
</Note>
...
<Note>
- Replace `<your_api_key>` with your actual API key. You can generate one by following the steps in the <a href="/sections/quickstart" target="_blank" rel="noopener noreferrer">Quickstart guide</a>.
