...
- **pipeline\_mode: "full"**: Enables all system features, including speech recognition, visual analysis, natural-sounding speech, and real-time language understanding.
- **system\_prompt**: Tells the agent to act professionally and respond helpfully, while being aware of the user’s emotional state.
...
- **Persona Layer**:
  ...
  - **TTS Layer**: Uses the `cartesia` engine to speak naturally and with emotion. The agent’s voice can sound calm, confident, or supportive based on the situation.
  - **STT Layer**: Uses the `tavus-advanced` engine to accurately convert speech to text. It’s tuned to detect pauses and interruptions, allowing smooth and natural conversation.
