...
| `language` | Defines the conversation language. Set to `multilingual` to enable automatic language detection via ASR, allowing the system to recognize and respond in the user's spoken language. See the [Language Support](/sections/conversational-video-interface/language-support) article for supported options. |
