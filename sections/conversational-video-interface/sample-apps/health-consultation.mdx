...
This application demonstrates a simple videocall with a [Health Care Consultant](/sections/conversational-video-interface/conversation/usecases/health-care) persona. The application uses Tavus CVI to create an interactive AI host that responds to user inputs and performs actions through tool calls.
## Prerequisites
