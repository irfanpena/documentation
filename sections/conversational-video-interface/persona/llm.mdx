...
Tavus-hosted `tavus-llama` has a **hard limit of 16,000 tokens**. If this limit is exceeded, the conversation may crash.

To avoid this, we **recommend keeping input context within 10,000 tokens** as a safe threshold.

- 1 token = 4 characters on average.
- 10,000 tokens = 40,000 characters (including whitespace and punctuation).
...
* A `/chat/completions` route but your `base_url` does not include any extension
...
Base URL of your LLM endpoint. Do not include route extensions.

```json
"base_url": "https://your-llm.com/api/v1/chat/completions"
```
