...
A **persona** defines how a replica behaves, speaks, and responds. It bundles identity, knowledge, and CVI pipeline settings to create a real-time conversational agent.
The Conversational Video Interface (CVI) pipeline is modular and fully configurable through the Persona settings. You can enable, disable, or customize individual layers and choose a pipeline mode that best suits your use case.
...
<Card title="Echo mode quickstart" icon="keyboard" href="/sections/conversational-video-interface/persona/llm#custom-llms" />
