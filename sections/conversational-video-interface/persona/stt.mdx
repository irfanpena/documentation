...
The STT Layer in Tavus allows your persona to transcribe and understand spoken input in real time. It supports dynamic and responsive conversation flow with intelligent turn-taking.
...
<Note>
By default the `smart_turn_detection` is set to `true`.
</Note>
This setting activates dynamic turn-taking based on real-time speech patterns. Powered by the Sparrow-0 model, it evaluates pauses, interruptions, and speech content to create smooth, natural conversation flow.

```json
"smart_turn_detection": true
```
<Warning>
Setting `smart_turn_detection` to `false` disables the use of the **Sparrow-0** model in the STT layer. The system will fall back to a fixed timeout-based method for detecting turn transitions, which is less adaptive to user behavior.
</Warning>
## Example Use Case
