...
```
#### How `smart_turn_detection` Works

When enabled, `smart_turn_detection`:

* Monitors participant speech in real time
* Detects when the participant has likely finished speaking
* Works with both speculative and non-speculative inference
* Adjusts response timing based on speech rhythm and pauses
* Integrates with `participant_pause_sensitivity` to avoid interrupting mid-thought
