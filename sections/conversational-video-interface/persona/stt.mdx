...
<Note>
- The **default engine** for the STT layer is `tavus-advanced`, which supports multilingual transcription with high accuracy.
- For faster response times, you can switch to `tavus-turbo`, though it may offer slightly less precise results compared to `tavus-advanced`. 
</Note>
```json
"layers": {
  "stt": {
...
```
<Warning>
Disabling `smart_turn_detection` deactivates **Sparrow** and reverts to a fixed timeout method for turn transitions, which is less adaptive.
</Warning>
## Example Use Case
