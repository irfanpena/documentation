...
'x-api-key: <api-key>'
```
For example, you are using the <a href="/api-reference/conversations/create-conversation" target="_blank">POST - Create Conversation</a> endpoint to create a real-time video call session with a Tavus replica that has been configured with a persona. In this scenario, you can send an API request adding your API Key (`<api_key>`) as shown below:
```shell cURL
curl --request POST \
